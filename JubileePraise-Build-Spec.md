# JubileePraise.com — Build Specification
...
JubileePraise.com is an internal, editor-gated catalog and operations console for managing every musical asset produced under the Jubilee Software, Inc. umbrella — across the Inspire Family persona ecosystem, affiliated artists, children's brands (Party Giggles, Tiny Tiggles), and other Jubilee initiatives.
...
JubileePraise is the **workshop**. The CDN is the **storefront**. Postgres holds the working state; the CDN holds the canonical published state.
...
```mermaid
flowchart TB
    User[Content Editor] -->|SSO Login| JV[JubileePraise.com<br/>Next.js Frontend]
...
- **Operational plane** — Postgres backing JubileePraise — holds editorial state: pipeline assignments, ratings, comments, nominations, audit logs. The workshop floor.
...
Authentication is delegated entirely to **JubileeInspire SSO** at `api.JubileeInspire.com` via OAuth 2.0 / OpenID Connect. JubileePraise does not manage passwords, MFA, or account lifecycle directly.

**Login flow:**

1. Unauthenticated user hits any JubileePraise route → redirect to `/auth/login`
...
A user must have a minimum role of **`content_editor`** to access any part of JubileePraise.
...
JubileePraise is the source of truth for **all** Jubilee Radio programming. The Jubilee Radio Engine v3.0 (Icecast-KH + Liquidsoap on Ubuntu 24.04 LTS) reads playlist manifests from `cdn.jubileeverse.com/radio/`.
...
1. JubileePraise regenerates the canonical JSON manifest
...
A sticky audio player sits at the bottom of every JubileePraise page, persisting across navigation.
...
The `/admin` section is gated to `admin` role. It provides operational visibility across the entirety of JubileePraise.
...
- **CORS** — allow JubileePraise origin
...
Published audio is publicly cacheable (TTL 24h+). Unpublished audio requires an editor session token signed by JubileePraise — Worker verifies signature against shared secret or JWT public key.
...
4. **Suno integration** — direct API integration for triggering generation from JubileePraise, or manual upload of generated assets
5. **Streaming distribution handoff** — does JubileePraise push to DSPs, or hand off to a third-party aggregator (DistroKid, etc.)?
6. **Cortex Flow integration** — how tightly does the JubileePraise pipeline integrate with the existing Cortex Flow release state machine?
