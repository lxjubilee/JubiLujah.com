// Reference client for JubileeInspire -> JubileePraise server-to-server calls.
//
// Implements the client-credentials flow against api.jubileepraise.com: fetches a
...
//   JUBILEEPRAISE_API_BASE       (default https://api.jubileepraise.com)
//   JUBILEEPRAISE_CLIENT_ID      (the id registered in JubileePraise's SERVICE_CLIENTS)
//   JUBILEEPRAISE_CLIENT_SECRET  (that client's secret)
//   JUBILEEPRAISE_SCOPE          (default "admin.set_password admin.provision")

const BASE = process.env.JUBILEEPRAISE_API_BASE || 'https://api.jubileepraise.com';
const CLIENT_ID = process.env.JUBILEEPRAISE_CLIENT_ID;
const CLIENT_SECRET = process.env.JUBILEEPRAISE_CLIENT_SECRET;
const SCOPE = process.env.JUBILEEPRAISE_SCOPE || 'admin.set_password admin.provision';
...
async function fetchToken() {
  if (!CLIENT_ID || !CLIENT_SECRET) {
    throw new Error('JUBILEEPRAISE_CLIENT_ID / JUBILEEPRAISE_CLIENT_SECRET are not configured');
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(`JubileePraise token request failed: ${res.status} ${detail}`);
  }
...
// Set an existing JubileePraise account's password. Returns { ok: true } on success.
...
// Create a JubileePraise account. 201 = created, 409 = already exists — both are
