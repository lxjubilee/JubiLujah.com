# JubileePraise.com — Operational Requirements & Single Source of Truth

**Project:** JubileePraise.com
**Owner:** Gabriel Ungureanu, Jubilee Software, Inc.
**Status:** Living document — supersedes scattered notes; complements `JubileePraise-Build-Spec.md`
**Last Updated:** 2026-06-04

This document captures every operational requirement the owner has stated, every architectural decision currently in force, and every rule the engineering pipeline must honor. When `JubileePraise-Build-Spec.md` describes *what is being built*, **this document describes *how the catalog actually runs day-to-day*.** If the two disagree, this document wins for operational behavior — the spec wins for the v1 build target.
...
|---|---|---|
| **C:** | **Retired — nothing for this workspace lives on C:** | The old `c:\Websites\jubileepraise.com\` authoring root **does not exist and is not to be recreated**. Working files, scratch and session artifacts belong on **W:**. The only C: presence is the Claude Code CLI's own runtime temp dir, whose location is set by the CLI, not by this project. |
| **J:** | Production CDN backing store | **`J:\jubileepraise.com\music\`** — 14,241 files / ~41 GB, and the content the live CDN serves. This is the source tree for `deploy/rebuild-manifest.mjs`, `deploy/check-manifest.mjs`, `_r2-sync-music-inspire.js` and the `ARTWORK_BASE` scripts. Master manifest: `J:\jubileepraise.com\music\catalog-manifest.json`. |
| **W:** | Repo + web serving | **`W:\JubileePraise.com\`** is the repo (authoring, `app/web`, `deploy/`). `W:\jubileepraise.com\public\` serves the local web server on port 3119. Audio always comes from the CDN, never from W:. Session scratch lives at `W:\.claude-scratch\`. |
...
- **`C:` = `c:\Websites\jubileepraise.com\` authoring root.** Retired. PUBLISH.md already recorded on
  2026-07-23 that this path does not exist on the machine.
- **`J:` = `J:\music\`, and "NEVER use `J:\jubileepraise.com\` — that path is forbidden (caused
  multi-phase sync confusion previously)."** Inverted by the Founder on 2026-08-14: **`J:\music\`
  does not exist**, the entire 41 GB store has always been at **`J:\jubileepraise.com\music\`**, and the
...
| Audio assets (`.mp3`) | CDN: `https://cd.jubilujah.com/music/<category>/<artist>/<CODE-slug>/tracks/*.mp3` (backed by `J:\jubileepraise.com\music\`). **No `albums/` segment on the CDN** — the bundled web manifest carries it and `lib/cdn.ts` strips it. |
...
**Server MIME types** must include `.mp3`, `.m4a`, `.aac`, `.wav`, `.flac`, `.ogg`, `.webp` — already added to `W:\jubileepraise.com\server.js`.
...
- **`cd.jubilujah.com`** is the live Cloudflare-fronted endpoint, backed by R2 and staged from `J:\jubileepraise.com\music\`. *(Corrected 2026-08-14: this section previously named `cdn.jubileeverse.com`, which returns 404 for music. That host is the **avatars** bucket `jubileeverse-cdn`; the only R2 credentials on the machine are scoped to it, which is why the CDN sync in PUBLISH.md Step 1 is currently blocked.)*
...
<header class="site-header">
  <div class="container">
    <div class="topbar">
      <a href="/index.html" class="site-logo">JubileePraise.com</a>
...
`J:\jubileepraise.com\music\catalog-manifest.json` is the **master** (paths stored as `inspire/…`) and
`W:\JubileePraise.com\app\web\public\music\catalog-manifest.json` is the **bundled web copy** (same data,
...
`c:\Websites\jubileepraise.com\music\_quick-manifest.js` does not exist on this machine)*. It scans every
...
cd c:\Websites\jubileepraise.com
node music/_quick-manifest.js

# 2. Copy to W: so the served pages immediately reflect new state
cp music/catalog-manifest.json W:/jubileepraise.com/public/music/

# 3. (Optional) Push to J: for CDN backing if R2 sync is hooked up
cp music/catalog-manifest.json J:/jubileepraise.com/music/
```
...
3. Copy the rebuilt manifest to W: (`W:\jubileepraise.com\public\music\catalog-manifest.json`)
...
1. *(Retired — the C: authoring root no longer exists. New albums are authored directly into `J:\jubileepraise.com\music\`.)* Any copy **into** the J: store stays additive (`robocopy /E`) — **never /MIR**.
...
- All HTML pages under `W:\jubileepraise.com\public\` (~19 pages site-wide as of 2026-06-04)
- Three JS assets under `W:\jubileepraise.com\public\web\_assets\`:
...
- Generator: `c:\Websites\jubileepraise.com\music\_quick-manifest.js`
- Server: `W:\jubileepraise.com\server.js` (port 3119)
- Companion spec: `JubileePraise-Build-Spec.md` (v1 engineering target — describes the future Postgres + FastAPI + Next.js system; this document describes what's running today)
